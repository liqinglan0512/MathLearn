      ...
      <div className="mt-8 space-y-3">
        {articles.map((a) => (
          <Link
            key={a.id}
            to={`/principles/${a.id}`}
            className="block rounded-lg border border-white/10 p-5 transition-colors hover:border-white/25 sm:p-6"
          >
            <Badge variant="outline" className="text-xs">{a.topic}</Badge>
            <h2 className="mt-2.5 text-lg font-medium leading-snug">{a.title}</h2>
            <p className="mt-1.5 text-sm text-neutral-400">{a.summary}</p>
        ...
        {articles.length === 0 && (
          <p className="rounded-lg border border-dashed border-white/10 py-16 text-center text-sm text-neutral-400">
            还没有推导长文，来写第一篇。
          </p>
        )}
      </div>
