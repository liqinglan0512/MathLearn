...
> **状态更新（2026-09-06）**：本文是 0.3 开发轮结束时的历史记录，其中的数字与待办反映当时状态，保留不改。
> 之后的公开发布审计见 [`../RELEASE_READINESS_REPORT.md`](../RELEASE_READINESS_REPORT.md)。
> 其中原本待决的许可证问题已经落定（代码 MIT、内容 CC BY-SA 4.0），`recharts` 已移除，测试数由 100 增至 103。
...
清理提交：`2d65102b408a3e9bf08f448484b47bdf79c01e08`。

- 删除 1 份已经失去作用的模板说明 `template-info.md`；其余 14 份 Markdown 分别承担项目入口、贡献规范、产品方向、内容模型、权限、来源、测试或版本证据职责，因此保留；
- 删除 45 个未被任何运行时代码或测试引用的通用 UI 模板组件，以及未引用的 `src/App.css` 和 `src/hooks/use-mobile.ts`；
- 删除本机调试截图目录、一次性调试脚本、临时样式和 Python cache，合计约 58 MB；
- 16 个测试文件、100 项测试全部保留。测试不是临时垃圾，而是产品边界、数据模型和回归行为的可执行证据；
- 《几何原本》的 607 个 entry、621 个懒加载 JSON、7,152 个 SemanticBlock、依赖边、来源问题、生成脚本、编辑器、历史实现和测试全部保留；
- 没有删除 `node_modules`，因为本地质量门仍依赖它；没有修改 `package.json` 或 `package-lock.json`，也没有借清理之名进行依赖升级；
- 清理后再次执行完整质量命令和生产浏览器 QA，结果仍为 PASS。

删除判断以“没有调用方、可再生且没有来源/历史/测试职责”为门槛。无法确认可安全删除的文件一律保留。
