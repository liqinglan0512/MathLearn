...
| `lodash@4.17.21` | high；[GHSA-r5fr-rjxr-66jc](https://github.com/advisories/GHSA-r5fr-rjxr-66jc)、[GHSA-f23m-r3pf-42rh](https://github.com/advisories/GHSA-f23m-r3pf-42rh)、[GHSA-xxjr-mmjv-4gpg](https://github.com/advisories/GHSA-xxjr-mmjv-4gpg) | transitive，经 `recharts@2.15.4` | runtime | **当前否** | `npm audit --omit=dev` 唯一告警。清理未引用模板组件后，源码已没有 Recharts import；现有 `dist` JavaScript 也没有 `recharts`/`lodash` 标记。因此当前静态客户端不可达，但依赖仍在 manifest 中，未来重新引入图表会改变此判断。 | `4.18.1`，满足 Recharts 的 `^4.17.21`；不需要升级 Recharts 3。升级后仍需构建和图表 QA。 |
