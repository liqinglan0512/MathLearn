...
`check:hygiene` 检查构建产物、依赖目录、浏览器 profile、调试文件和本地任务状态没有被 Git 跟踪；它不会删除任何文件。
...
《几何原本》保留可追溯的英文底本、历史中文材料和机器辅助现代中文。机器内容必须保持其真实状态，不能自动升级为 `math_reviewed` 或 `published`。数学内容的最终判断保留给人工编辑。
